...
import ru.Art3m1y.delivery_service.adapter.in.dto.CreateOrderRqDto;
import ru.Art3m1y.delivery_service.application.port.in.*;
...
public class OrderController {
    ...
    public void createDeliveryFromOrder(@PathVariable UUID orderId) {
        createDeliveryUseCase.createDeliveryFromOrder(CreateDeliveryCommand.builder()
    ...
    public void cancelDeliveryByOrder(@PathVariable UUID orderId) {
        cancelDeliveryUseCase.cancelDeliveryFromOrder(CancelDeliveryCommand.builder()
                .id(new OrderId(orderId))
                .build());
    }

    public CreateOrderCommand mapToCreateOrderCommand(CreateOrderRqDto requestDto) {
        return new CreateOrderCommand(new Order(null, mapItems(requestDto.getItems()),
                mapRecipient(requestDto.getRecipient()),
                mapAddress(requestDto.getAddress()),
                null));
    }
